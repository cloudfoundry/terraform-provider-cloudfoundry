...
resource "cloudfoundry_service_instance" "xsuaa_svc" {
  ...
  service_plan = [for plan in data.cloudfoundry_service_plans.xsuaa_svc.service_plans : plan.id if plan.name == "application"][0]
  parameters   = <<EOT
  {
  "xsappname": "tf-test2",
...
resource "cloudfoundry_service_instance" "dev-autoscaler" {
  ...
  space        = data.cloudfoundry_space.team_space.id
  service_plan = [for plan in data.cloudfoundry_service_plans.autoscaler_svc.service_plans : plan.id if plan.name == "standard"][0]
  timeouts = {
    create = "10m"
  }
}
