...
func TestResourceServiceRouteBinding(t *testing.T) {
	...
	// Tests the cascading dependency chain: space -> data_source -> service_instance + route -> route_binding.
	// When allow_ssh changes on the space, the data source is deferred (depends_on),
	// making all its attributes unknown. This cascades through:
	// data.space.id -> service_instance.space + route.space -> their IDs -> binding attributes.
	t.Run("happy path - route binding not replaced when space allow_ssh changes", func(t *testing.T) {
		testResourceNotReplacedOnSpaceUpdate(t,
			...
			func(allowSSH bool) string {
				return hclSpaceWithSSHAndDataSource("test-space-route-binding-stability", allowSSH) + `
