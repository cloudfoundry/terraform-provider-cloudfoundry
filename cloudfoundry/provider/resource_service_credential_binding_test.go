...
func TestResourceServiceCredentialBinding(t *testing.T) {
	...
	// Tests the cascading dependency chain: space -> data_source -> service_instance -> credential_binding.
	...
	// data.space.id -> service_instance.space -> service_instance.id -> binding.service_instance.
	t.Run("happy path - credential binding not replaced when space allow_ssh changes", func(t *testing.T) {
		testResourceNotReplacedOnSpaceUpdate(t,
			...
			func(allowSSH bool) string {
				return hclSpaceWithSSHAndDataSource("test-space-binding-stability", allowSSH) + `
