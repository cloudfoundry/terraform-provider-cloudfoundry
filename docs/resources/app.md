...
- `lifecycle_type` (String) The lifecycle type used to stage the application. Common values include `buildpack` and `docker`. Defaults to `docker` when `docker_image` is set, otherwise `buildpack`. Support for additional lifecycle identifiers depends on the target Cloud Foundry platform.
